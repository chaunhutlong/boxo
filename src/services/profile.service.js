...
const { getSignedUrl, deleteFileFromS3FromS3 } = require('../utils/s3');
...
const createOrUpdateProfile = async (userId, avatar, profileBody) => {
  try {
    ...
    if (profile) {
      ...
      if (avatar) {
        await deleteFileFromS3FromS3(BUCKET, profile.avatar);
    ...
    if (avatar) {
      await deleteFileFromS3FromS3(BUCKET, avatar.key);
    }
